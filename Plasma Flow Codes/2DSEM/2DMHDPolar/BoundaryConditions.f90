...
subroutine TwoDBCApply(BCMatrix,GEMatrix,SysMatrix,Nx,Ny)
	!! This function generates system matrix
	integer i,j
	!! the start and end indices
	integer jstart,jend
	...
	! the boundary conditions in x direction
	do i=1,Ny
		jstart=(i-1)*Nx+1
		jend=i*Nx
		do j=1,Nx
			if(j==1 .or. j==Nx) then
				!SysMatrix(jstart+j-1,jstart:jend)=BCMatrix(jstart+j-1,jstart:jend)
				SysMatrix(jstart+j-1,:)=BCMatrix(jstart+j-1,:)
			else
				!SysMatrix(jstart+j-1,jstart:jend)=GEMatrix(jstart+j-1,jstart:jend)
				SysMatrix(jstart+j-1,:)=GEMatrix(jstart+j-1,:)
			end if
		end do
	end do
	...
	do j=2,Nx-1
		SysMatrix(j,:)=BCMatrix(j,:)
		SysMatrix(j+(Ny-1)*Nx,:)=BCMatrix(j+(Ny-1)*Nx,:)
	end do


	
end subroutine TwoDBCApply
