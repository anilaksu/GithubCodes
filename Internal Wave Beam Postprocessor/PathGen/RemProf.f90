do j=1,Num
    ...
    call interpVal(IR(1,:,:),PressInt(j,1,:),Press(j,:),Point(1,:,:1),Point(1,:,2),nxr,nzr,Np(1),20)
 
    call interpVal(IR(1,:,:),UInt(j,1,:),u(j,:),Point(1,:,:1),Point(1,:,2),nxr,nzr,Np(1),20)

    call interpVal(IR(1,:,:),VInt(j,1,:),v(j,:),Point(1,:,:1),Point(1,:,2),nxr,nzr,Np(1),20)
...
open(413,file='HEnergyFluxPyc1.dat',status='unknown')
...
open(415,file='PEnergyFluxPyc2.dat',status='unknown')
...
open(416,file='HEnergyFluxPyc1.dat',status='unknown')
...
print*,xi(j),zi(j),PointRef(1,j,1),PointRef(1,j,2)
end do 
